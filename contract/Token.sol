...
contract Token is AppStorageFacet, ERC1155StorageFacet {
    ...
    function addToken(
        ...
        require(basicPrice > 0, "basicPrice must gt 0");
    ...
    function isTokenOwner(uint tokenId, address ownerAddress) public view returns (bool) {
        ERC1155FacetStorage storage ds = erc1155Storage();
        uint[] memory tokenIdArr = ds._tokenOwnByMap[ownerAddress];
        for (uint i = 0; i < tokenIdArr.length; i++) {
            if (tokenIdArr[i] == tokenId) {
                return true;
            }
        }
        return false;
